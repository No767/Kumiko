...
class Jobs(commands.Cog):
    ...
    async def file(
        ...
        status = await self.pool.execute(
            query, ctx.guild.id, ctx.author.id, name.lower(), True
        )  # type: ignore
        if status[-1] == 0:
            await ctx.send(MessageConstants.NO_JOB.value)
        else:
    ...
    async def unfile(
        ...
        """
        status = await self.pool.execute(
            query, ctx.guild.id, ctx.author.id, name.lower(), False
        )  # type: ignore
        if status[-1] == 0:
            await ctx.send(MessageConstants.NO_JOB.value)
        else:
    ...
    async def apply(
        ...
        async with self.pool.acquire() as conn:
            ...
            rows = await conn.fetchrow(
                "SELECT creator_id, worker_id FROM job WHERE guild_id = $1 AND name = $2;",
                ctx.guild.id,
                name.lower(),
            )  # type: ignore
            # customizable?
            ...
            status = await submit_job_app(
                ctx.author.id, ctx.guild.id, name.lower(), False, conn
            )  # type: ignore
            await ctx.send(status)
    ...
    async def quit(
        ...
        async with self.pool.acquire() as conn:
            ...
            if dict(rows)["worker_id"] is None:
                ...
                status = await submit_job_app(
                    None, ctx.guild.id, name.lower(), True, conn
                )  # type: ignore
                await ctx.send(status)
