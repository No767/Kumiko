...
class Github(commands.Cog):
    ...
    async def issues(
        ...
        async with (
            self.session.get(issues_url, headers=self.headers) as issues_res,
            self.session.get(comments_url, headers=self.headers) as comments_res,
        ):
    ...
    async def repo(self, ctx: commands.Context, owner: str, repo: str) -> None:
        ...
        async with (
            self.session.get(repo_url, headers=self.headers) as repo_res,
            self.session.get(releases_url, headers=self.headers) as releases_res,
        ):
