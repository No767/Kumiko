...
class SpigetV2(commands.Cog):
    ...
    async def on_message(self, ctx, *, search:str):
        ...
        file_size = str(resource[0]['file']['size']) + str(resource[0]['file']['sizeUnit'])
        download_url_false = "https://spigotmc.org/" + str(resource[0]['file']['url'])   
        try:
            if str(resource[0]['file']['type']) == 'external':
                embedVar = discord.Embed()
                embedVar.add_field(name="Plugin Info", value=f"Name >> {resource[0]['name']}\nTag >> {resource[0]['tag']}\nAuthor >> {resourcev2[0]['name']}\nDownloads >> {resource[0]['downloads']}\nRating >> {resource[0]['rating']['average']}", inline=False)
                embedVar.add_field(name="Tested Versions", value=str(resource[0]['testedVersions']).replace("[", "").replace("]", "").replace("'", ""), inline=False)
                embedVar.add_field(name="Supported Languages", value=f"{resource[0]['supportedLanguages']}", inline=False)
                embedVar.add_field(name="Latest Plugin Version", value=f"{resourcev3[0]['name']}", inline=False)
                embedVar.add_field(name="Downloads", value=f"Type >> {resource[0]['file']['type']}\nSize >> {file_size}\nURL >> {resource[0]['file']['url']}", inline=False)
                ...
                embedVar.add_field(name="Supported Languages", value=f"{resource[0]['supportedLanguages']}", inline=False)
                embedVar.add_field(name="Latest Plugin Version", value=f"{resourcev3[0]['name']}", inline=False)
                embedVar.add_field(name="Downloads", value=f"Type >> {resource[0]['file']['type']}\nSize >> {file_size}\nURL >> {download_url_false}", inline=False)
                embedVar.set_thumbnail(url=str(thumbnail))
                await ctx.send(embed=embedVar)
