...
class ErrorHandler(commands.Cog):
    ...
    async def on_command_error(
        ...
        if isinstance(error, commands.CommandNotFound):
            ...
            embedVar = discord.Embed(
                color=discord.Color.from_rgb(226, 199, 255))
            embedVar.description = f"You are missing the following permissions: {error.missing_perms}"
            msg = await ctx.send(embed=embedVar, delete_after=10)
            await msg.delete(delay=10)
            ...
            embedVar = discord.Embed(
                color=discord.Color.from_rgb(226, 199, 255))
            embedVar.description = f"You are missing the following roles: {error.missing_roles}"
            msg = await ctx.send(embed=embedVar, delete_after=10)
            await msg.delete(delay=10)
