...
class JikanV1(commands.Cog):
    ...
    async def anime(self, ctx, *, search: str):
        ...
        try:
            ...
            embedVar.add_field(
                name="Title", value=anime_info["results"][0]["title"], inline=False
            )
            embedVar.add_field(
                name="Synopsis",
                value=anime_info["results"][0]["synopsis"],
                inline=False,
            )
            embedVar.add_field(
                name="Episodes",
                value=anime_info["results"][0]["episodes"],
                inline=False,
            )
            embedVar.add_field(
                name="Score", value=anime_info["results"][0]["score"], inline=False
            )
            embedVar.add_field(
                name="Start Date (airing)",
                value=anime_info["results"][0]["start_date"],
                inline=False,
            )
            embedVar.add_field(
                name="End Date (airing)",
                value=anime_info["results"][0]["end_date"],
                inline=False,
            )
            embedVar.add_field(
                name="MAL ID", value=anime_info["results"][0]["mal_id"], inline=False
            )
            embedVar.add_field(
                name="MAL URL", value=anime_info["results"][0]["url"], inline=False
            )
