...
class View(discord.ui.View):
    ...
    @discord.ui.button(
        label="Yes", row=0, style=discord.ButtonStyle.primary, emoji=discord.PartialEmoji.from_str("<:check:314349398811475968>")
    )
    ...
    @discord.ui.button(label="No", row=0, style=discord.ButtonStyle.primary, emoji=discord.PartialEmoji.from_str("<:xmark:314349398824058880>"))
...
class ecoUsers(commands.Cog):
    ...
    eco_users = SlashCommandGroup(name="users", description="Commands for handling user-related data and transactions for Kumiko's economy system", guild_ids=[970159505390325842])
    ...
    async def ecoUserTransaction(self, ctx, *, user: Option(str, "The user you wish to trade with"), amount: Option(int, "How much coins you wish to make in the transaction")):
        ...
        await utilsUser.userTransaction(sender_id=senderID, receiver_id=sortedReceiver, sender_amount=totalAmountSender, receiver_amount=totalAmountReceiver)
        embed = discord.Embed()
        embed.description = "Transaction made!"
        embed.add_field(name="Total balance for sender", value=totalAmountSender, inline=True)
        embed.add_field(name="Total balance for receiver", value=totalAmountReceiver, inline=True)
