...
class DeviantArtV1(commands.Cog):
    ...
    async def da(self, ctx, *, deviation_id: str):
        async with aiohttp.ClientSession(json_serialize=orjson.dumps) as session:
            params = {"with_session": "false", "limit": "5", "access_token": f"{DeviantArt_API_Access_Token}"}
            async with session.get(f"https://www.deviantart.com/api/v1/oauth2/deviation/{deviation_id}", params=params) as r:
...
class DeviantArtV2(commands.Cog):
    ...
    async def da_query(self, ctx, *, search: str):
        ...
        async with aiohttp.ClientSession(json_serialize=orjson.dumps) as session:
            params = {"q": f"{search}", "with_session": "false", "limit": 10, "mature_content": "False", "access_token": f"{DeviantArt_API_Access_Token}"}
            async with session.get("https://www.deviantart.com/api/v1/oauth2/browse/newest", params=params) as resp:
                art = await resp.json()
                try:
...
class DeviantArtV3(commands.Cog):
    ...
    async def deviantart_popular(self, ctx, *, search: str):
        ...
        async with aiohttp.ClientSession(json_serialize=orjson.dumps) as session:
            params = {"q": f"{search}", "with_session": "false", "limit": "10", "mature_content": "false", "access_token": f"{DeviantArt_API_Access_Token}"}
            async with session.get("https://www.deviantart.com/api/v1/oauth2/browse/popular", params=params) as response:
...
class DeviantArtV4(commands.Cog):
    ...
    async def tags(self, ctx, *, search: str):
        ...
        async with aiohttp.ClientSession(json_serialize=orjson.dumps) as session:
            params = {"tag": f"{search}", "with_session": "false", "limit": "10", "mature_content": "false", "access_token": f"{DeviantArt_API_Access_Token}"}
            async with session.get("https://www.deviantart.com/api/v1/oauth2/browse/tags", params=params) as rep:
                tags = await rep.json()
                try:
    ...
    async def on_command(self, ctx=None):
        getTokens()
        print(f"Access Token (right before the command was invoked): {getTokens[0]}\nRefresh Token (right before the command was invoked): {getTokens[1]}")
...
class DeviantArtV5(commands.Cog):
    ...
    async def user(self, ctx, *, search: str):
        async with aiohttp.ClientSession(json_serialize=orjson.dumps) as session:
            params = {"ext_collections": "false", "ext_galleries": "false", "with_session": "false", "mature_content": "false", "access_token": f"{DeviantArt_API_Access_Token}"}
            async with session.get(f"https://www.deviantart.com/api/v1/oauth2/user/profile/{search}", params=params) as respon:
